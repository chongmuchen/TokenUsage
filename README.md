...
- 每日配额图使用与顶部周限额卡相同的 Codex Home 和周限额桶，仅受日期筛选影响。第一张图可切换“每天开始剩余配额”（`100 − 起点已用%`）与“当天累计使用配额”；第二张图显示截至观测点的本地 API USD 等价金额除以当天累计使用的配额百分点，单位为 USD/1%。正常日一个点；同日发生重置时按前后周期分段显示多个点，累计使用量相加，不把旧周期未用完的部分当作消耗。日初优先沿用前一日最后观测；缺少相邻观测、旧报告或价格覆盖不完整时标记近似，无法计算的比值留空。历史报告可通过“同步当前日期范围”补充每日观测。
