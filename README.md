...
- 普通刷新只读 `token-usage/reports/*.json`，并只读 `state_*.sqlite` 的 `first_user_message` 来生成短标题。周限额百分比与重置时间由本地解析器从 transcript 的 `token_count.rate_limits` 白名单字段提取后随报告落盘；同一精确重置值只保留最后观测，秒级抖动再由 App 聚类。App 在报告实际加载时于后台计算当前预估和最近历史，不为此查询网络，也没有周限额轮询器。
...
- 趋势页顶部默认显示“当前周限额预估”，使用服务端限额快照中的 7 天窗口，而不是“最近一周”日期快捷项。它将本次限额周期截至快照时刻的本地 Token/API USD 等价除以已用百分比，粗略折算用满 100% 时的总量；当前日期、模型、推理强度和速度筛选不会改变该卡片。下方历史表保留最近若干个已结束的服务端周窗口，显示 100% 配额估值与最后观测的实际用量、使用率和截止时间；它不是自然周账单，也不会为了更新历史持续扫描 transcript 或请求 API。多个 Codex Home 会分别估算，界面选取当前快照最新的一组，不跨目录合计。
