...
- 普通刷新只读 `token-usage/reports/*.json`，并只读 `state_*.sqlite` 的 `first_user_message` 来生成短标题；会话“项目”优先使用明确关联的项目名，否则使用 `cwd` 的末级目录名（完整路径只用于本地悬停提示）。周限额百分比与重置时间由本地解析器从 transcript 的 `token_count.rate_limits` 白名单字段提取后随报告落盘。周限额预估仍对同一精确重置值只保留最后观测；每日配额趋势另保留本地日与 UTC 日的首末观测和重置边界，App 对秒级重置时间抖动聚类。App 在报告实际加载时于后台计算当前预估和最近历史，不为此查询网络，也没有周限额轮询器。
...
- “同步当前日期范围”会运行随 App 打包的本地解析器：为缺失报告的用户会话生成 per-session JSON/TXT，并重建缺少分钟样本、限额快照或每日配额观测、仍是下界、曾因计数校验而抑制价格、或使用旧价目快照的已有报告；内容完整且使用当前价目的报告会跳过。它不会调用 Codex 模型或 OpenAI API，也不会覆盖实时的 `latest.json/latest.txt`。
...
- 每日配额图使用与顶部周限额卡相同的 Codex Home 和周限额桶，仅受日期筛选影响。第一张图可切换“每天开始剩余配额”（`100 − 起点已用%`）与“当天累计使用配额”；第二张图显示截至观测点的本地 API USD 等价金额除以当天累计使用的配额百分点，单位为 USD/1%。正常日一个点；同日发生重置时按前后周期分段显示多个点，累计使用量相加，不把旧周期未用完的部分当作消耗。点位放在所属日期内，具体观测时间在悬停提示中显示，避免接近午夜的观测被误看作次日。日初优先沿用前一日最后观测；缺少相邻观测、旧报告或价格覆盖不完整时标记近似，无法计算的比值留空。历史报告可通过“同步当前日期范围”补充每日观测。
...
- `thread.usage` 已扣除 fork 时继承的父上下文前缀，是该线程自身用量，不含后代线程。新解析器还会在 fork 的首个 Token 快照只计入该次请求用量，避免把继承的累计计数误记到分叉当天；旧 fork 报告可通过历史同步重建。
